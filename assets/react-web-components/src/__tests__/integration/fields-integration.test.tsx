...
describe("GetFields API + FieldRenderer 統合テスト", () => {
  describe("APIレスポンスからのフィールド描画", () => {
    it("文字列フィールド（UIType 1）が正しく描画される", () => {
      ...
      expect(
        screen.getByPlaceholderText("顧客企業名を入力してください"),
      ).toBeInTheDocument();
    });
  ...
  describe("全フィールドの一括描画", () => {
    it("複数フィールドを順番に描画できる", () => {
      const fields = mockGetFieldsResponse.fields.map(
        convertApiFieldToDefinition,
      );
  ...
  describe("読み取り専用フィールド", () => {
    it("readonly=trueのフィールドが無効化される", () => {
      ...
      const { container } = render(
        <FieldRenderer field={field} value="test" onChange={onChange} />,
      );
