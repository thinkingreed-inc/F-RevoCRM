...
describe("usePicklistDependency", () => {
  describe("hasDependency", () => {
    it("連動設定が存在する場合はtrueを返す", () => {
      const { result } = renderHook(() =>
        usePicklistDependency(mockDependency),
      );
  ...
  describe("getFilteredFields", () => {
    ...
    it("連動設定がないフィールドはそのまま返す", () => {
      ...
      expect(leadsourceField?.picklistValues).toEqual(
        mockFields[0].picklistValues,
      );
    });
  });
  ...
  describe("isValueAllowed", () => {
    it("許可された値に対してtrueを返す", () => {
      ...
      const formData = { leadsource: "Web" };
      expect(result.current.isValueAllowed("industry", "IT", formData)).toBe(
        true,
      );
      expect(
        result.current.isValueAllowed("industry", "Software", formData),
      ).toBe(true);
    });
    ...
    it("許可されていない値に対してfalseを返す", () => {
      ...
      const formData = { leadsource: "Web" };
      expect(
        result.current.isValueAllowed("industry", "Finance", formData),
      ).toBe(false);
      expect(
        result.current.isValueAllowed("industry", "Healthcare", formData),
      ).toBe(false);
    });
    ...
    it("空値は常にtrueを返す", () => {
      ...
      const formData = { leadsource: "Web" };
      expect(result.current.isValueAllowed("industry", "", formData)).toBe(
        true,
      );
      expect(result.current.isValueAllowed("industry", null, formData)).toBe(
        true,
      );
      expect(
        result.current.isValueAllowed("industry", undefined, formData),
      ).toBe(true);
    });
    ...
    it("連動設定がないフィールドはtrueを返す", () => {
      ...
      const formData = { leadsource: "Web" };
      expect(
        result.current.isValueAllowed("nonexistent", "any", formData),
      ).toBe(true);
    });
  });
  ...
  describe("getFieldsToClear", () => {
    it("ソースフィールド変更時に許可されない値を持つターゲットフィールドを返す", () => {
      ...
      const fieldsToClear = result.current.getFieldsToClear(
        "leadsource",
        "Referral",
        formData,
      );
