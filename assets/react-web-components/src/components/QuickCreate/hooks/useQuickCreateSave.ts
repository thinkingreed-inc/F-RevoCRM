...
export function useQuickCreateSave(
  ...
  const save = useCallback(
    async (formData: Record<string, any>): Promise<QuickCreateSaveResult> => {
      setIsSaving(true);
      setError(null);
      ...
      try {
        ...
        let processedData = syncJoditEditorFormData(
          module,
          formData,
          fields || [],
        );
        ...
        if (isCalendarModule) {
          ...
          if (processedData.recurringEditMode) {
            bodyParams.append(
              "recurringEditMode",
              String(processedData.recurringEditMode),
            );
          }
        }
        ...
        if (response.status === 401) {
          throw new Error(
            "セッションがタイムアウトしました。ページをリロードして再度ログインしてください。",
          );
        }
        ...
        const recordLabel =
          data._recordLabel || (data as any).result?._recordLabel;
