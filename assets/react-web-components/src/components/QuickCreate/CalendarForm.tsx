...
export interface CalendarFormProps {
  ...
  parseReminderValue: (value: FieldValue) => {
    days: number;
    hours: number;
    minutes: number;
  };
...
export const CalendarForm: React.FC<CalendarFormProps> = ({
  ...
  const [isInviteeDropdownOpen, setIsInviteeDropdownOpen] =
    useState<boolean>(false);
  const inviteeDropdownRef = useRef<HTMLDivElement>(null);
  const inviteeInputContainerRef = useRef<HTMLDivElement>(null);
  const [inviteeDropdownPosition, setInviteeDropdownPosition] = useState<{
    top: number;
    left: number;
    width: number;
  } | null>(null);
  const [inviteeHighlightedIndex, setInviteeHighlightedIndex] =
    useState<number>(0);
   ...
   */
  const isAllDay =
    formData["is_allday"] === true ||
    formData["is_allday"] === "1" ||
    formData["is_allday"] === "true";
  ...
  useEffect(() => {
    if (
      initialSelectedInvitees &&
      initialSelectedInvitees.length > 0 &&
      !initialInviteesLoadedRef.current
    ) {
  ...
  const { subject, dateStart, dueDate, reminderTime, othersByBlock } =
    categorizeFields(currentFields);
  ...
  const handleReminderChange = useCallback(
    (field: "days" | "hours" | "minutes", value: number) => {
      ...
      const newValue = combineReminderValue(
        updated.days,
        updated.hours,
        updated.minutes,
      );
    ...
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      switch (e.key) {
        ...
        case "Enter":
          ...
          e.preventDefault();
          if (
            inviteeHighlightedIndex >= 0 &&
            inviteeHighlightedIndex < inviteeCandidates.length
          ) {
        ...
        <div className="flex-1 min-w-0">
          <div className="flex gap-2">
            ...
            <Input
              ...
              value={date}
              onChange={(e) => {
                const newValue = combineDateTimeValue(
                  e.target.value,
                  shouldShowTime ? time : "",
                );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 pr-8 pb-2">
        ...
        {activeTab === "Events" && (
          <div className="flex items-start gap-2">
            ...
            <span
              className="w-3 leading-[30px] flex-shrink-0"
              aria-hidden="true"
            />
      ...
      <div className="space-y-3">
                ...
                {isAlarmBlock && (
                  <div className="md:col-span-2">
                    <div className="flex items-start gap-2">
                      ...
                      <span
                        className="w-3 min-h-[54px] pt-[18px] flex-shrink-0"
                        aria-hidden="true"
                      />
                      <div
                        className={cn(
                      ...
                      >
                        <div className="flex items-center gap-2 h-[30px] mb-0">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              handleReminderToggle(checked === true)
                            }
                            disabled={isDisabled}
                          />
                          <label
        ...
        {reminderTime &&
          !Object.keys(othersByBlock).includes(reminderTime.block || "") && (
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0 flex items-center h-[30px]">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      onFieldChange("send_mail", checked ? "1" : "0")
                    }
                    disabled={isDisabled}
                  />
                </div>
