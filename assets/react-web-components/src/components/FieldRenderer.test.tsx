...
describe("FieldRenderer", () => {
  describe("文字列フィールド (UIType 1, 2, 106)", () => {
    it("文字列入力フィールドが正しく表示される", () => {
      ...
      expect(
        screen.getByPlaceholderText("テスト文字列を入力してください"),
      ).toBeInTheDocument();
    });
  ...
  describe("日時フィールド (UIType 6)", () => {
    it("日時入力フィールドが正しく表示される", () => {
      const field = createField({
        uitype: UI_TYPES.DATETIME_CALENDAR,
        label: "日時",
        name: "datetime_field",
      });
      const onChange = vi.fn();
  ...
  describe("製品税フィールド (UIType 83)", () => {
    ...
    it("値がある場合はチェックボックスがチェック状態になる", () => {
      ...
      render(
        <FieldRenderer field={field} value="10.000" onChange={onChange} />,
      );
  ...
  describe("フィールド情報エラー", () => {
    it("フィールドがnullの場合エラー表示", () => {
      ...
      render(
        <FieldRenderer field={null as any} value="" onChange={onChange} />,
      );
