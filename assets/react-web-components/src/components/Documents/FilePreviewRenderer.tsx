...
function isPreviewable(
  ...
  if (category === "image" || category === "video" || category === "audio")
    return true;
  if (
    ["pdf", "xlsx", "pptx", "docx", "odt", "ods", "odp", "fodt"].includes(ext)
  )
    return true;
  if (
    category === "text" ||
    ["txt", "csv", "ics", "log", "json", "xml"].includes(ext)
  )
    return true;
  return false;
}
...
const DocxPreview: React.FC<{
  ...
  useEffect(() => {
    ...
    fetch(`index.php?module=Documents&action=PreviewContent&record=${recordId}`)
      ...
      .then((data) => {
        if (data.success && data.result?.html) setHtml(data.result.html);
      })
      .catch(() => setHtml(null))
