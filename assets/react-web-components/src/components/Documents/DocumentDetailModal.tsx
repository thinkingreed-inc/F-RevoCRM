...
function getComplianceStatusStyle(
  t: (key: string) => string,
  status: string,
...
function getFieldTone(
  ...
  if (field.name === "compliance_status") {
    if (rawValue === "compliant")
      return { pillBg: "#e7f4e8", pillColor: "#2e8b46" };
    if (rawValue === "non_compliant")
      return { pillBg: "#FED7D7", pillColor: "#822727" };
  }
  if (field.name === "input_deadline_status") {
    if (rawValue === "within")
      return { pillBg: "#e7f4e8", pillColor: "#2e8b46" };
    if (rawValue === "warning")
      return { pillBg: "#fdf1e0", pillColor: "#a86a12" };
    if (rawValue === "overdue")
      return { pillBg: "#FED7D7", pillColor: "#822727" };
  }
  if (field.name === "document_category")
    return { pillBg: "#e7eefb", pillColor: "#3461bd" };
      ...
      {isEmpty ? (
        ...
        (() => {
          ...
          if (
            ...
            return refId ? (
              <a
                href={`index.php?module=${refModule}&view=Detail&record=${refId}`}
                style={{
                  fontSize: 14,
                  color: "#3b7dd8",
                  textDecoration: "none",
...
const BlockCard: React.FC<{
  ...
  >
    ...
    >
      ...
      />
      <span style={{ fontSize: 14, fontWeight: 700, color: "#2b2f33" }}>
        {block.label}
      </span>
    </div>
    <div
  ...
  >
    ...
    >
      <div style={{ flex: 1 }}>
        ...
        >
          {record.module_label || record.module}
        </div>
          ...
          <div style={{ fontSize: 12, color: "#A0AEC0", marginTop: 2 }}>
            ...
            {record.summary.amount && (
              <span style={{ marginLeft: 8 }}>
                {record.summary.currency_symbol}
                {Number(record.summary.amount).toLocaleString()}
              </span>
            )}
          </div>
        )}
...
export const DocumentDetailModal: React.FC<DocumentDetailModalProps> = ({
  ...
  const getDynamicFieldValue = (fieldName: string): any => {
    ...
    if (
      docAny.dynamic_fields &&
      docAny.dynamic_fields[fieldName] !== undefined &&
      docAny.dynamic_fields[fieldName] !== null
    )
      return docAny.dynamic_fields[fieldName];
  ...
  const blocks = useMemo((): BlockDef[] => {
    ...
    const blockMap: Record<
      string,
      { label: string; accent: string; fields: BlockDef["fields"] }
    > = {};
    ...
    const coreInfo: BlockDef["fields"] = [
      ...
      },
      {
        ...
        rawValue: doc.assigned_user_name,
      },
      {
        field: {
          name: "createdtime",
    ...
    const fileFields: BlockDef["fields"] = [
      {
        field: {
          ...
          blockLabel: "",
          picklistValues: [
            { value: "I", label: t("LBL_LOCATION_FILE") },
            { value: "E", label: t("LBL_LOCATION_URL") },
          ],
        } as DocFieldInfo,
        rawValue: doc.filelocationtype,
      },
      {
        field: {
          name: "filestatus",
      ...
      },
      {
        ...
        rawValue: formatFileSize(doc.filesize),
      },
      {
        field: {
          name: "fileversion",
    ...
    for (const f of fieldDefs) {
      ...
      if (!blockMap[blockKey]) {
        ...
        const accentKey =
          Object.keys(BLOCK_ACCENTS).find((k) => t(k) === f.blockLabel) || "";
        blockMap[blockKey] = {
          label: f.blockLabel,
          accent: BLOCK_ACCENTS[accentKey] || DEFAULT_ACCENT,
          fields: [],
        };
      }
      const isWide =
        ...
        f.uitype === "21" ||
        f.name === "file_hash" ||
        f.name === "compliance_notes";
      blockMap[blockKey].fields.push({
        field: f,
        rawValue: rawVal,
        wide: isWide,
      });
  ...
  if (doc) {
    ...
    if (doc.compliance?.document_category) {
      ...
      const catLabel =
        catField?.picklistValues?.find((pv) => pv.value === catKey)?.label ||
        (
          {
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            {doc?.title || t("LBL_DOCUMENT_DETAIL")}
          </div>
        </div>
        <div style={{ flex: 1, overflowY: "auto" }}>
          {isLoading ? (
            ...
            >
              {doc.compliance?.compliance_status === "non_compliant" &&
                doc.compliance?.compliance_notes && (
                  <div
                    style={{
                      padding: "8px 12px",
                      background: "#FFF5F5",
              ...
              {blocks.map((b) => (
                <BlockCard
                  key={b.label}
                  block={b}
                  dynamicFields={doc.dynamic_fields}
                  t={t}
                />
              ))}
              {/* Related records */}
              ...
              >
                ...
                >
                  ...
                  />
                  <span
                    style={{ fontSize: 14, fontWeight: 700, color: "#2b2f33" }}
                  >
                    {t("LBL_RELATED_RECORDS")}
                  </span>
                </div>
                <div style={{ padding: "12px 16px" }}>
                  {doc.related_records && doc.related_records.length > 0 ? (
                    ...
                    >
                      <div style={{ fontWeight: 600, marginBottom: 2 }}>
                        {t("LBL_NO_RELATED_RECORD")}
                      </div>
                      <div>{t("LBL_RELATE_REQUIRED_MSG")}</div>
                    </div>
                  ) : (
                    <div style={{ fontSize: 12, color: "#A0AEC0" }}>
                      {t("LBL_NO_RELATED_RECORD_SHORT")}
                    </div>
                  )}
                </div>
              </div>
        ...
        <ComplianceHistoryModal
          isOpen={showHistory}
          title={doc?.title || ""}
          fileVersions={doc?.file_versions || []}
    ...
    >
      <div
        style={{
          ...
          display: "flex",
          flexDirection: "column",
          boxShadow: "0 24px 60px rgba(20,28,40,.16)",
          overflow: "hidden",
      ...
      >
        ...
        >
          <span style={{ fontSize: 14, color: "#7b828a", fontWeight: 500 }}>
            {folderPathStr}
          </span>
          <div style={{ flex: 1 }} />
          {doc && (
                  ...
                  >
                    ...
                    <div style={{ padding: "12px 16px" }}>
                      {doc.related_records && doc.related_records.length > 0 ? (
                        ...
                        <div
                          style={{ fontSize: 13, color: "#A0AEC0", padding: 4 }}
                        >
                          {t("LBL_NO_RELATED_RECORD_SHORT")}
                        </div>
                      )}
