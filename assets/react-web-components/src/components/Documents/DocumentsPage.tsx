...
const DocumentsPageInner: React.FC<DocumentsPageProps> = ({
  ...
  const [folderDialogTarget, setFolderDialogTarget] = useState<Folder | null>(
    null,
  );
        ...
        {!isMobile && (
          ...
          >
            ...
            {breadcrumb.map((item) => (
              <React.Fragment key={item.id}>
                ...
                <span
                  ...
                  style={{
                    cursor: "pointer",
                    color: item.id === selectedFolderId ? "#2D3748" : "#4299E1",
                    fontWeight: item.id === selectedFolderId ? 600 : 400,
                  }}
                >
                  {item.name}
                </span>
            ...
            {filterType === "starred" && (
              <>
                <span style={{ color: "#CBD5E0" }}>&gt;</span>
                <span style={{ fontWeight: 600 }}>{t("LBL_STARRED")}</span>
              </>
            )}
          </div>
        )}
