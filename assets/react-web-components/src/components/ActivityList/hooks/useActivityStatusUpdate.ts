...
export interface UseActivityStatusUpdateResult {
  /** ステータス更新関数 */
  updateStatus: (activityId: string, fieldName: string, newValue: string, activityType: string) => Promise<void>;
  /** 更新中フラグ */
  isUpdating: boolean;
  /** エラーメッセージ */
  error: string | null;
...
export function useActivityStatusUpdate(): UseActivityStatusUpdateResult {
  ...
  const updateStatus = useCallback(async (
    ...
    try {
      // CSRF トークンを取得
      const csrfInput = document.querySelector('input[name="__vtrftk"]') as HTMLInputElement;
      ...
      const errorMessage = err instanceof Error ? err.message : 'ステータスの更新中にエラーが発生しました';
      setError(errorMessage);
