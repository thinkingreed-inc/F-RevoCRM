...
describe("useTranslation", () => {
  describe("t 関数", () => {
    ...
    it("%s プレースホルダーを1つ置換", () => {
      ...
      expect(result.current.t("LBL_FIELD_REQUIRED", "タイトル")).toBe(
        "タイトルは必須です",
      );
    });
  ...
  describe("translations", () => {
    ...
    it("DEFAULT_TRANSLATIONS が含まれる", () => {
      ...
      expect(result.current.translations.LBL_SAVE).toBe(
        DEFAULT_TRANSLATIONS.LBL_SAVE,
      );
      expect(result.current.translations.LBL_CANCEL).toBe(
        DEFAULT_TRANSLATIONS.LBL_CANCEL,
      );
    });
  });
  ...
  describe("実際のユースケース", () => {
    it("バリデーションメッセージの生成", () => {
      ...
      // 日付バリデーションエラー
      expect(t("LBL_END_DATE_AFTER_START")).toBe(
        "終了日時は開始日時より後に設定してください",
      );
    });
    ...
    it("招待者選択UIの文字列", () => {
      ...
      expect(t("LBL_ALL_USERS_SELECTED")).toBe(
        "すべてのユーザーが選択済みです",
      );
