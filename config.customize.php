...
// ドキュメントのプレビュー可能な上限（バイト）
// これを超えるファイルはプレビューせず、ダウンロードを促す。
// 大きなファイルは pdf.js / Luckysheet の解析やサーバー側の変換で
// メモリと時間を使い切り、画面が固まる原因になるため制限している。
// 0 以下を指定すると制限しない（非推奨）。既定は 20MB。
$documents_preview_maxsize = 20971520;// 20MB
