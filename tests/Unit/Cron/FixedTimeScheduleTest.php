...
final class FixedTimeScheduleTest extends TestCase
{
    ...
    {
        return mktime(10, 7, 33, 8, 25, 2026);
    }
    ...
    public function test_I2_指定時刻を過ぎていれば翌日になる(): void
    {
        ...
        self::assertSame(
            mktime(10, 7, 0, 8, 26, 2026),
            Vtiger_Cron::computeNextDailyRunAt(10 * 60 + 7, mktime(10, 7, 0, 8, 25, 2026)),
            'I2 境界: ちょうど指定時刻 → 翌日'
        );
    }
    ...
    public function test_I11_月末指定は月の長さに追従する(): void
    {
        ...
        self::assertSame(
            mktime(9, 0, 0, 2, 28, 2027),
            Vtiger_Cron::computeNextMonthlyRunAt(
                Vtiger_Cron::RUN_ON_LAST_DAY_OF_MONTH,
                9 * 60,
                mktime(10, 0, 0, 2, 1, 2027)
            ),
            'I11 月末指定（2027 年 2 月・平年）→ 2/28'
        );
        self::assertSame(
            mktime(9, 0, 0, 2, 29, 2028),
            Vtiger_Cron::computeNextMonthlyRunAt(
                Vtiger_Cron::RUN_ON_LAST_DAY_OF_MONTH,
                9 * 60,
                mktime(10, 0, 0, 2, 1, 2028)
            ),
    ...
    public function test_I12_その月に無い日を指定したら末日になる(): void
    {
        self::assertSame(
            mktime(9, 0, 0, 2, 28, 2027),
            Vtiger_Cron::computeNextMonthlyRunAt(31, 9 * 60, mktime(10, 0, 0, 2, 1, 2027)),
    ...
    {
        self::assertSame(
            mktime(9, 0, 0, 1, 5, 2027),
            Vtiger_Cron::computeNextMonthlyRunAt(5, 9 * 60, mktime(10, 0, 0, 12, 20, 2026)),
    ...
    public function test_遅れてもグリッドへ復帰し遅れを持ち越さない(): void
    {
        ...
        $finished = mktime(19, 59, 30, 8, 25, 2026);
        self::assertSame(
            mktime(20, 0, 0, 8, 25, 2026),
