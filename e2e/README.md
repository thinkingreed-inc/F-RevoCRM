...
実証スペック: `tests/3_共通機能/3-28_権限.spec.ts`（可視範囲）/ `tests/3_共通機能/3-29_権限_アクション.spec.ts`
（アクション権限）/ `tests/3_共通機能/3-30_権限_項目.spec.ts`（項目レベル権限）/ `tests/3_共通機能/3-31_権限_出力.spec.ts`
（エクスポート/インポート権限）/ `tests/3_共通機能/3-32_共有ルール.spec.ts`（カスタム共有ルール）/
`tests/3_共通機能/3-27_所有者変更.spec.ts`（所有者変更）/ `tests/3_共通機能/3-18_タグ絞り込み.spec.ts`（タグ絞り込み）/
`tests/3_共通機能/3-05_リスト条件.spec.ts`（CustomView 絞り込み条件）/ `tests/3_共通機能/3-06_ページング.spec.ts`（ページング・ソート）/
`tests/3_共通機能/3-02_絞り込み.spec.ts`（検索・絞り込み）。組織共有の状態は設定画面でも `tests/5_管理設定/C-04_共有アクセス.spec.ts` が確認。
