...
- [ ] **`C-01_ユーザー` は後始末が無効化のみで、実行を重ねると自テストが壊れる**
  作成したユーザーを削除しないため、ローカルで反復実行すると一覧 1 ページ目(20 件)が
  e2e ユーザーで埋まり、「追加したユーザーが一覧に出ない」で落ちる（ワークフローと同じパターン。
  CI は毎回クリーンな dump なので顕在化しない）。列検索で絞ってから確認する形に直すのが堅牢。
...
- [ ] skip 中の残り: D-06 入力制限（本 checkout にモジュール未導入。導入可否の判断が先）
