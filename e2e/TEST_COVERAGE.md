...
| ~~`tests/7_モジュール管理/`~~ | - | **2026-08-28 に廃止**。38 モジュール総当たりで検証していたのは「ModuleManager のトグル → `vtiger_tab.presence` 更新 → メニュー再生成」というモジュール非依存の共通経路のため、`tests/5_管理設定/D-01_モジュール管理.spec.ts` に代表 1 モジュール(顧客企業)で集約した。**⚠️ 顧客企業を一時的に無効化するため他 spec と並列実行できない。CI では `CI_SERIAL_SPECS`(2 段目 / workers=1)で実行し、1 段目は `--grep-invert '管理: モジュール管理'` で除外する** | ✅ |
...
>
> **インポート(`import.create`)**: describe から必須項目を解決してフラット CSV(名前列 + 必須項目、2 行)を生成 → `utils/import.ts::runImport`(モジュール可変・重複処理ステップの有無を `getUnsupportedDuplicateHandlingModules` 相当集合で分岐・ワーカー横断の `withImportLock` で直列化・停止(HALTED)取り込みの滞留を `mode=clearCorruptedData` で自己修復)で 3 ステップウィザードを実行 → Webservice API で作成 2 件を確認 → プレフィックス前方一致で削除して冪等化(取り込み実行を try/finally で囲み、途中失敗でも後始末を必ず実行)。必須の関連項目(reference)は参照先の既存レコードの表示名を CSV 値に充てて名前解決させる(`MatrixTest.resolveReferenceCsvValue`: Users=user_name / Currency=currency_name / その他=labelFields 連結)。**インポート対応(run=green, 13)**: Accounts / Contacts / Potentials / Leads / HelpDesk / Products / Vendors / ServiceContracts / Services / Project(フラット項目のみ)+ ProjectMilestone・ProjectTask(projectid→Project の projectname を充当)/ Dailyreports(reports_to_id→admin の user_name を充当)。**na(14)**: 非インポート(Documents / Faq / Campaigns / Assets / EmailTemplates / PDFTemplates / Portal / SMSNotifier)、明細+reference 必須のインベントリ(Invoice / Quotes / SalesOrder / PurchaseOrder)、.ics 専用フローの Calendar、PriceBooks(必須 currency_id は currency_name で解決するが本 DB の通貨名が "Japan, Yen" のようにカンマを含み Import の参照パーサがカンマ分割して解決不能。加えて CSV 取り込みが 1 行目だけ作成して停止=IMPORT_STATUS_HALTED しキューロックが残留)。skip(2): NewModule / Approval(プレースホルダ)。MatrixTest 側でも必須 reference/未対応型(time/multipicklist 等)に当たれば `UnconfiguredCaseError`(理由付き skip)へ退避。2 連続実行で 15 pass(setup 2 + 13 モジュール) / 16 skip / 0 fail・残留レコード 0 を確認。
>
...
| 11-1 | CSV インポート | `modules/Import/` | ✅ | `tests/3_共通機能/3-23_インポート.spec.ts`（Accounts の重複処理: 複数行/ヘッダなし/スキップ/上書き/マージ）+ `tests/3_共通機能/3-24_インポート履歴.spec.ts`（履歴確認）+ **`tests/2_CRUD/2-1_マトリクス.spec.ts` の `import.create` でインポート対応 13 モジュールを網羅**（describe 駆動で CSV 生成→作成→API 確認→削除。必須 reference は参照先の表示名で名前解決）。3-23/3-24 は Accounts 固有の重複処理・履歴の深掘りとして維持 |
...
| 機能 | 実装 | メモ |
|---|---|---|
| ドキュメント Documents | `modules/Documents` | ファイルプレビュー / DL / 整合性チェック / メール添付。**機能一覧に未記載だがモジュール有効**（実機のメニュー・関連一覧に「ドキュメント」表示を確認）。**固有機能のテストは #1659 のドキュメント刷新後に設計する**（2026-08-27 判断） |
...
- [x] **`tests/7_モジュール管理` を CI へ復帰（DB 直接接続を廃止）** ※2026-08-28 に `D-01_モジュール管理` へ集約し、本 suite は廃止
  後始末が runner 上で `php -r` を実行して DB へ直接接続していたため、CI では `db_server` が
  compose のサービス名 `db` で名前解決できず `getaddrinfo for db failed` で必ず失敗していた。
  DB 直接操作の用途は「元の presence 読み取り」「復帰セーフティネット」「残存検出」の 3 つだったが、
  トグル自体は元々 ModuleManager の UI 経由で、各テストの finally でも UI で復帰していたため
  afterAll だけが DB に依存していた。**afterAll を UI ベースに統一**し(`support/db.ts` は削除)、
  一覧のチェックボックスで確認 → 無効なら UI で復帰 → それでも残るなら対象名と復旧手順
  (管理画面で有効化 / `npm run e2e:reset`)を添えて失敗させる形にした。
  判断の根拠: E2E の目的は壊れの検知であり、後続を無理に通すことではない。UI が壊れていれば
  このテスト自身が落ちて検知でき、CI は毎回クリーンな dump なので次の実行に影響しない。
...
- [x] **`C-03_プロファイル` の「プロファイルの複製」の flaky は解消済み**（2026-08-27 確認）。
  flaky が出たのは初回 CI（16 分 49 秒の回）だけで、当時はカレンダー 3 本が 3 回リトライして
  CI 全体が高負荷だった。その負荷が消えた後の 4 回の CI ではすべて一発 pass（6.6s / 7.9s / 7.5s、flaky 0）。
  保存待ちは既に `action=Save` の POST レスポンスを待つ実装になっているため追加対処は不要。
- [x] **CI でクイック作成モーダルの必須ピックリストが埋まらない → 原因特定・解消**
  カレンダーの `createEventViaModal` を使う 3 本（`2_モーダル` / `3_繰り返し` / `4_招待`）が
  CI で「ステータスは必須です」「活動タイプは必須です」で保存できなかった。
  **真因**: `Calendar_QuickCreateRecordStructure_Model::getStructure()` は
  `eventstatus` / `activitytype` の既定値を **ログインユーザーの個人設定**
  （`vtiger_users.defaulteventstatus` / `defaultactivitytype`）から取る。
  `vtiger_field.defaultvalue` は空でフォールバックも効かないため、素のインストール状態
  （未設定）ではモーダルから保存できない。CI は毎回クリーンな dump なので常に未設定だった。
  → `seed-spec.json` に `adminPreferences` を追加し、`seed_e2e_data.php` で admin に
  `defaulteventstatus=Planned` / `defaultactivitytype=Call` を設定して dump を再生成した。
  **調査中に判明した重要な落とし穴**: それまで「ローカルではクリーン DB でも通る」と判断していたが、
  これは誤りだった。`db_password` が空の環境で `mysql -p""` を使うと対話プロンプトになり、
  stdin にリダイレクトした dump の 1 行目をパスワードとして読んで **認証失敗して投入されない**。
  そのため「reset したつもりで実は汚れた DB のまま」テストしていた（個人設定は過去の
  `I-01` / `I-02` 実行で入っていた）。dump を扱うスクリプトは
  `--defaults-extra-file` を使うこと（`reset-local-db.sh` も同じ書き方なので要修正）。
- [x] **`C-01_ユーザー` の一覧依存を除去**（2026-08-27）。
  ユーザーは vtiger の仕様上削除できず（`Users/actions/DeleteAjax.php` は認証情報削除用）、
  E2E が作ったユーザーは実行ごとに増える。一覧は 1 ページ 20 件なので、素朴に
  「一覧を開いて token で filter」すると作成直後の行が 1 ページ目に出ず落ちていた。
  → 行の特定は **必ず列検索（`listSearch(page, "user_name", …)`）で絞ってから**行い、
  record id は一覧の行属性ではなく **API（`SELECT id FROM Users WHERE user_name = …`）**で引くようにした。
  検証: e2e ユーザーが 12 → 13 に増えた状態でも 6 passed。
...
- [x] **`reset-local-db.sh` の dump 投入が壊れていたのを修正**（2026-08-27）。
  `db_password` が空の環境で `mysql -p"$DB_PASS"` を使うと mysql が対話プロンプトを出し、
  stdin にリダイレクトした dump の 1 行目をパスワードとして読んで認証失敗する。
  しかも失敗が無視されて先へ進むため「reset したつもりで実は投入されていない」状態になっていた
  （実際にこれで長時間「クリーン DB でも通る」と誤認した）。
  → 接続情報を一時 `--defaults-extra-file` 経由に変更（空パスワードでも安全・コマンドラインに露出しない）。
  併せて `localhost → 127.0.0.1` の変換をやめ（権限が `user@localhost` のみの環境で
  TCP 接続が Access denied になるため config の値をそのまま使う）、
  投入失敗時は即 exit し、投入後のテーブル数が異常に少なければエラーにする検証も入れた。
  検証: これまで失敗していた `npm run e2e:reset` が完走し、migration が 24 件スキップ（= dump が最新）になった。

### CI 対象の整理と追加（2026-08-28）

「CI で実行する内容」を業務的に意味のある単位へ寄せた。

**追加**
- **共通機能: インポート** → `tests/3_共通機能/3-23_インポート.spec.ts` を CI_SPECS に追加。
  新規作成 / ヘッダなし CSV / 重複時の **スキップ・上書き・マージ** を一通り通す
  （spec は既にあったが CI に入っていなかった）。1 ユーザーにつき同時実行できない
  （ロック）ため spec 内は `describe.serial`。実測 35 秒。
- **レポート: 新規作成 → 実行結果の確認 → 削除** → `4-10_レポート.spec.ts` の
  「一括削除」テストを拡張。3 ステップ UI で作成したレポートを開いて
  `#countValue > 0` と明細に実データ（API で用意した顧客担当者）が出ることまで確認し、
  そのうえで削除する。
- **選択肢: 登録 → レコード作成 → 削除（付け替え確認）** → `D-04_ピックリスト.spec.ts`。
  案件のタイプに選択肢を 2 つ作り、一方を使う案件を作成 → その選択肢を削除する際に
  もう一方を「置き換え先」に指定 → **案件の値が置き換え先へ UPDATE される**ことを API で確認。
  *知見*: 削除モーダル `#replaceValue` の option の value は内部 ID、表示テキストは
  vtranslate される（`Existing Business` → 「既存ビジネス」）。DB 値を厳密に検証するには
  置き換え先も**テスト内で作った値**（翻訳辞書に無い＝表示 = DB 値）にする必要がある。
- **選択肢の連動設定 → レコード作成画面での動作確認** → `D-05_ピックリスト依存.spec.ts`。
  案件の フェーズ → タイプ で連動を作り、連動表で連動先を 1 つだけ残して保存 →
  作成画面で連動元を選ぶと**連動先の option が残した 1 値だけに絞られる**ことを確認。
  連動は保存後 `Vtiger/resources/Edit.js` の picklistDependency が
  連動元の change で連動先の option を差し替える実装なので、設定画面だけでは担保できない。
- **文言変更 → 実画面で文言が変わることの確認** → `D-07_言語変換.spec.ts`。
  対象モジュールを **キャンペーン(Campaigns)** に限定してルールを作り、
  キャンペーン一覧で変換後の文言が出ること、ルール削除で元に戻ることを確認。
  対象を `common`（全モジュール）にすると全画面に影響して並列実行中の他 spec を
  巻き込むため限定する。「キャンペーン」は他 spec がセレクタに使っていない文言。

**削除・集約**
- **マイサイト(Portal) の spec を廃止** — 機能 OFF 予定のため
  `4-12_マイサイト.spec.ts` を削除し CI_SPECS からも除いた。
- **`tests/7_モジュール管理/`（38 モジュール総当たり）を廃止し `D-01_モジュール管理` に集約** —
  検証しているのは「トグル → `vtiger_tab.presence` 更新 → メニュー再生成」という
  **モジュール非依存の共通経路**で、モジュールごとに繰り返す価値が薄い（2 段目で 1.7 分）。
  代表 1 モジュール（顧客企業）で「無効化 → メニューから消える → 再有効化 → 戻る」を確認する。
  顧客企業を一時的に無効化するため 1 段目（並列）では走らせず、
  `CI_SERIAL_SPECS` に入れて 2 段目（workers=1）で実行する。
  1 段目は `--grep-invert '管理: モジュール管理'` で二重実行を防ぐ
  （CI_SPECS は `tests/5_管理設定/` をディレクトリ指定しているため describe 名で除外する）。
...
- [x] **B3 MassDelete の個別実装** — Calendar / Reports / Portal / EmailTemplates / PDFTemplates を
  ...
  - ~~`tests/4_モジュール/4-12_マイサイト.spec.ts` — Portal(マイサイト)~~
    → **2026-08-28 に廃止**(機能 OFF 予定のため spec ごと削除)。
...
- [x] **P0-6 OSS 版に存在しない管理機能を恒久的な対象外として確定（2026-08-27 判断）**
  D-06 入力制限 / D-09 レコードタイプ / D-10 申請フィールド / D-11 承認フローは
  **OSS 版に搭載されていない**ため、テストは書かない。各 spec の skip 理由を
  「本環境には未導入」→「OSS 版には搭載されていないため恒久的に対象外」に正確化した
  （spec 自体は EE 版など搭載環境で使える形で残す）。`module=Approval` が空ページなのも同じ理由。
  → CI の静的 skip 13 件のうち 12 件はこれで説明がつく（残る 1 件は E-01 Webフォームの編集）。

> **E2E が 0 本のモジュール（2026-08-25 時点、機械的に確認）**: `Reports` / `MailManager` / `Emails` / `Google`。
> `tests/` 配下の spec で一度も言及されない（かつて `tests/7_モジュール管理/fixtures/modules.ts` のメニュー ON/OFF 対象として
...
> - 2 段目(workers=1): `tests/5_管理設定/D-01_モジュール管理.spec.ts`(モジュールをグローバルに ON/OFF するため単独実行が必須。2026-08-28 までは `tests/7_モジュール管理` がこの枠だった)。
...
> - **副作用として Invoice / Calendar はマトリクスの実行件数が 0 になったため、CI 代表から外した**。
>   capabilities の `applySkip(..., RECORD_DEPENDENT_CASES)` で「レコードを 1 件作る必要がある
>   ケース」が全て skip される(Invoice=明細必須で API/UI とも作成不可、Calendar=日時ウィジェット等の
>   特殊コントロールで作成不可)ため、残っていた run は CustomView 系だけだった。それを CI から
>   外した結果 15 件 collect / 15 件すべて実行時 skip になっていた。
>   collect だけ払って何も検証せず、カタログ上の「PRで実行」も過大に見えるので
>   `CI_SAMPLE_MODULES` から除外し、理由を `2-1_マトリクス.spec.ts` にコメントで明記した
>   (CI マトリクス: 122 → 92 collect、実行時 skip 39 → 9 件。実行数は 81〜83 で変わらない)。
>   機能面は `4-9_在庫/`(CRUD・明細・PDF・相互生成)と `4-8_カレンダー/`(7 spec)が担保。
>   ローカルのフル実行は全 29 モジュール対象のままなので、capability が run に変わった際の
>   検知はそちらで行う。
> - 結果(2026-08-28 時点 / P0-B 完了後): CI 実行対象は **265 テスト**(旧 179)。うち実測で実際に走るのは
>   **256 件**、残り 9 件はマトリクスの capability 判定による実行時 skip
>   (この内訳は `docs/e2e-catalog` に実測 json を取り込むと画面で確認できる)。
> - **CI 実測(run 33134299168 / 2026-08-28)**: **10 分 48 秒**(1 段目 = 並列 4 で 224 passed、
>   2 段目 = 直列で 32 passed)。失敗 0 / flaky 0。CPU 時間の合計は 1707 秒(28.5 分)で、
>   これを workers=4 で消化している。目標 20 分に対して十分な余裕がある。
>   超過するようになった場合の削減候補: マトリクス代表を 8 → 6 モジュールに戻す(-70 秒 wall)、
>   `5_管理設定` の一部を paths 条件付き実行に回す(ここが実行数で最大)。
> - **flaky の傾向と対処**(P0-B の CI 反復で判明。いずれも「反映待ちが足りない」系):
>   1) 一括削除・タグ保存などの **AJAX 完了前に page.goto すると POST が中断される**。
>      → まず「その場で対象行が消える / モーダルが閉じる」ことを待ってから開き直す。
>   2) URL の検索条件(`search_key`)は **Ajax 再読込で外れる**。件数(全行数)で判定せず
>      必ず対象の名前で絞る。全選択(`listViewEntriesMainCheckBox`)は他テストの
>      レコードまで消すため使わない。
>   3) 一覧の件数比較は並列実行中の他テストが件数を動かすため使わない(名前で判定する)。
>   4) 手数の多い spec(列検索 → 行メニュー → モーダル保存)は Playwright 既定の 30 秒では
>      足りない。`test.describe.configure({ timeout })` で明示的に伸ばす。
>   5) 一覧の全行を走査して 1 件を探すような実装は CI で時間切れになる。
>      dump 固定レコードを使い、差し替え検知の assert を添える。
...
- [x] CSV インポート（No.11-1） → `tests/3_共通機能/3-23_インポート.spec.ts`（Accounts、パターン別5種: 複数行/ヘッダなし/重複スキップ/上書き/マージ）。ウィザードは `utils/import.ts` で driver 化。**知見**: ヘッダは自動マップされず列順に明示割当が必要／重複突合は既定 `accountname`／スキップは未更新・上書き/マージは更新（本ビルドでは CSV に列自体が無い項目の空白化は起きず保持）。**他モジュール展開**: マトリクス `import.create`(§1)でインポート対応 13 モジュール(Accounts/Contacts/Potentials/Leads/HelpDesk/Products/Vendors/ServiceContracts/Services/Project + 必須 reference をシード参照名で充足した ProjectMilestone/ProjectTask/Dailyreports)を網羅済み。3-23/3-24 は Accounts 固有の重複処理・履歴の深掘りとして維持
...
- [x] 見積 → 請求 / 受注 / 発注 の相互生成（No.23-2 等）→ `tests/4_モジュール/4-9_在庫/4_相互生成.spec.ts`（P0-3 で実装）
- [x] PDF エクスポート（在庫系詳細）（No.5-1）→ `tests/4_モジュール/4-9_在庫/3_PDF.spec.ts`（P0-2 で実装。4 モジュール、DL したファイルの `%PDF-` まで確認）
- [ ] 繰り返し請求（受注・請求）
...
- [x] skip 解消: F-05 構成エディタ / H-01 税の管理（P0-5。H-01 は `dontTestFieldsName` で `taxN` を除外して根治）
...
- [x] Documents モジュールの扱い → **現時点ではトリアージしない**（2026-08-27 判断）。
  ドキュメント関連は #1659 で刷新が予定されているため、固有機能（ファイルプレビュー / DL /
  整合性チェック / メール添付）のテスト設計は刷新後に行う。
  なお共通 CRUD としてはマトリクスに含まれており CI でも一部実行されている
  （API で作成できるケースは run。UI フォームは `filelocationtype` 等で汎用ドライバ非対応のため skip）。
