...
- [ ] **CI では React クイック作成モーダルの必須ピックリストに既定値が入らない（原因未特定）**
  `createEventViaModal` を使う 3 本（`2_モーダル` / `3_繰り返し` / `4_招待`）が CI で保存できない。
  実測(3 回の CI 実行)で確認した事実:
  - モーダルは開くし、**担当=システム管理者 / 表示=公開 は既定値が入る**のに、
    **ステータス(eventstatus) と 活動タイプ(activitytype) だけが空**のまま。
  - そのため「ステータスは必須です」「活動タイプは必須です」で保存が弾かれる。
  - 20 秒待っても入らない（`waitModalRequiredDefaults` の poll がタイムアウト）ので
    「反映が遅い」ではなく**入らない**。
  - ローカル（クリーン DB + CI と同一のビルド成果物 + CI 相当の全体セット workers=4）では
    既定値が入り再現しない。
  - フィールドは「検索して選ぶ」型（`ステータスを検索...`）で、クリックや ArrowDown では
    候補が開かないため、テスト側から明示的に選ぶには値のハードコードが必要（環境依存になる）。
  仮説: ピックリスト選択肢の取得経路、または `vtiger_role2picklist` の割当が CI の
  dump + migration 後の状態で欠けている（CustomView の `cv2role`/`cv2rs` 欠落と同種の問題）。
  当面 CI からは除外し、モーダルを使わない 4 本（`1_基本` / `5_共有` / `6_活動ToDo` /
  `7_追加機能`）だけ CI に残す。**要調査**。
