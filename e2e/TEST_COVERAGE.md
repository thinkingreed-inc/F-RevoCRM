...
| `tests/4_モジュール/` | 15 | モジュール固有機能。`顧客企業/顧客担当者/リード/案件/発注先/チケット/テンプレート`(詳細固有アクション: メール・SMS・変換・作成起動・一覧/カスタマイズ) + `4-8_カレンダー/`(基本/モーダル(終日・共有メモ)/繰り返し/招待(複数ユーザー・3名招集・#864)/共有(可視性)/活動ToDo) + `4-9_在庫/`(在庫=Invoice/Quotes/SalesOrder/PurchaseOrder の CRUD + 割引/税/合計監査, 明細=インライン検索) | 🟡→✅ |
