...
| `tests/7_モジュール管理/` | 1 | Excel「3_〇_OSS版_モジュール管理.xlsx」判定=OK の 30 モジュールを対象に、ModuleManager UI(`input[name="moduleStatus"]`)でトグルし ON→メニューにリンク出現 / OFF→非出現 を検証(データ駆動: 1 モジュール 1 テスト)。メニューは描画場所別(サイドバー`#app-menu` / `.app-item-misc` / 上部ナビ)に判定。try/finally で各テストが UI 経由で有効へ戻し、afterAll でも一覧のチェックボックスを確認して残っていれば UI で復帰する(残る場合は対象名と復旧手順を添えて失敗)。Webforms は トップメニューにリンクが無く除外、メール/Webmails/承認アクション/Googleカレンダーは NA、申請/Excelテンプレートは OSS 非搭載で SKIP。**⚠️ 本 suite はグローバルにモジュールを ON/OFF する(共有CRMの `vtiger_tab.presence` を変更)ため、他 spec と並列実行すると干渉する。単独で(`-g "モジュール管理"` 等)または `--workers=1` で実行すること。CI では `CI_SERIAL_SPECS`(2 段目 / workers=1)で実行する** | ✅ |
...
- [x] **`tests/7_モジュール管理` を CI へ復帰（DB 直接接続を廃止）**
