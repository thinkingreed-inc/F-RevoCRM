...
class Vtiger_IFrameWidget_Model extends Vtiger_Widget_Model {
	...
	public function save($request) {
		...
		$dataValue = array();
		$dataValue['title'] = $title ? $title : 'IFrame Widget';
		$dataValue['url'] = $url ? $url : 'https://www.example.com';
		
		$data = Zend_Json::encode((object) $dataValue);
		$this->set('data', Vtiger_Util_Helper::toSafeHTML($data));
		
		$db->pquery('UPDATE vtiger_module_dashboard_widgets SET data=?, title=? WHERE id=?', 
			array($data, $dataValue['title'], $widgetId));
	}
