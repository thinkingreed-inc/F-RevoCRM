...
class SyncServer {
	...
	function put($key, $element, $user) {
		...
		foreach ($records as $record) {
			...
			$lookupResult = $db->pquery("SELECT serverid,clientmodifiedtime FROM vtiger_wsapp_recordmapping WHERE appid=? AND CAST(clientid AS CHAR CHARACTER SET utf8mb4) COLLATE utf8mb4_bin =?", array($appid, $clientRecordId));
