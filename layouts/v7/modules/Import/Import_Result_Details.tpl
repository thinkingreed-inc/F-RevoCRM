		...
		<td  class = "greenColor" width="60%">{$IMPORT_RESULT.CREATED}
			{if $IMPORT_RESULT['CREATED'] neq '0'}
				{if $FOR_MODULE neq 'Users'}
					&nbsp;&nbsp;&nbsp;&nbsp;<a class="cursorPointer" onclick="return Vtiger_Import_Js.showLastImportedRecords('index.php?module={$MODULE}&for_module={$FOR_MODULE}&view=List&start=1&foruser={$OWNER_ID}&_showContents=0&importid={$IMPORT_ID}')">{'LBL_DETAILS'|@vtranslate:$MODULE}</a>
				{/if}
			{/if}
		</td>
		...
		<tr>
			...
			<td width="30%">{$IMPORT_RESULT.SKIPPED}
				{if $IMPORT_RESULT['SKIPPED'] neq '0'}
					&nbsp;&nbsp;&nbsp;&nbsp;<a class="cursorPointer" onclick="return Vtiger_Import_Js.showSkippedRecords('index.php?module={$MODULE}&view=List&mode=getImportDetails&type=skipped&start=1&foruser={$OWNER_ID}&_showContents=0&for_module={$FOR_MODULE}&importid={$IMPORT_ID}')">{'LBL_DETAILS'|@vtranslate:$MODULE}</a>
				{/if}
			</td>
		</tr>
	...
	{if $IMPORT_RESULT['FAILED'] neq '0'}
		<tr>
			...
			<td width="30%"><font color = "red">{$IMPORT_RESULT.FAILED} / {$IMPORT_RESULT.TOTAL}</font>
				&nbsp;&nbsp;&nbsp;&nbsp;<a class="cursorPointer" onclick="return Vtiger_Import_Js.showFailedImportRecords('index.php?module={$MODULE}&view=List&mode=getImportDetails&type=failed&start=1&foruser={$OWNER_ID}&_showContents=0&for_module={$FOR_MODULE}&importid={$IMPORT_ID}')">{'LBL_DETAILS'|@vtranslate:$MODULE}</a>
			</td>
		</tr>
	{/if}
</table>