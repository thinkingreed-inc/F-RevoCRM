							...
							{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
								...
								{else if $FIELD_MODEL->get('uitype') eq "69" || $FIELD_MODEL->get('uitype') eq "105"}
									...
									<td class="fieldValue {$WIDTHTYPE}" colspan="3">
										<ul id="productsImageContainer">
											{foreach key=ITER item=IMAGE_INFO from=$IMAGE_DETAILS}
												{if !empty($IMAGE_INFO.url) && !empty({$IMAGE_INFO.orgname})}
												<li>
													<a href="{$IMAGE_INFO.url}" target="_blank" alt="{$IMAGE_INFO.orgname}" style="background-image: url({$IMAGE_INFO.url})">
														{* <img src="{$IMAGE_INFO.url}" title="{$IMAGE_INFO.orgname}" /> *}
													</a>
												</li>
												{/if}
											{/foreach}
