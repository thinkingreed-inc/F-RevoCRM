...
Vtiger.Class('Vtiger_Index_Js', {
	...
	showMap : function(e) {
		...
		app.request.post({"data":params1}).then(function(error,response) {
			...
			window.open("https://maps.google.co.jp/maps?q="+location);
			return ;
			...
			app.request.post({"data":params}).then(function(err,data){
				if(err === null){
					app.helper.loadPageOverlay(data,{'ignoreScroll' : true,'backdrop': 'static'}).then(function(){
						...
						vtUtils.showSelect2ElementView($('#overlayPage .data-header').find('select[name="taskstatus"]'),{placeholder:"ステータス : 全て"});
